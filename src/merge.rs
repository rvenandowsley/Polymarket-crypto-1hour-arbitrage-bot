...
/// RPC 限速时等待后重试的时长（略大于 "retry in 10s"），可通过 MERGE_RPC_RATE_LIMIT_BACKOFF_SECS 覆盖
...
/// 将多个 merge 调用编码为一次 proxy(calls[]) 调用，用于批量提交。
fn encode_proxy_calls_batch(ctf: Address, calldatas: &[Vec<u8>]) -> Vec<u8> {
    let calls: Vec<ProxyCallTuple> = calldatas
        .iter()
        .map(|data| ProxyCallTuple {
            typeCode: 1u8,
            to: ctf,
            value: U256::ZERO,
        ...
        .collect();
    proxyCall { calls }.abi_encode().to_vec()
}
...
async fn relayer_execute_merge(
    ...
    let gas_limit = gas_per_call * calldatas.len().max(1) as u64;
    ...
    // Polymarket Builder Secret 使用 base64url（含 _ 和 -），需用 URL_SAFE 解码
    let secret_bytes = base64::engine::general_purpose::URL_SAFE
        .decode(builder_secret.trim())
        .or_else(|_| base64::engine::general_purpose::STANDARD.decode(builder_secret.trim()))
...
pub async fn merge_max_batch(
    ...
    let mut merge_calldatas: Vec<Vec<u8>> = Vec::new();
    let mut merged_items: Vec<(B256, U256)> = Vec::new();

    let rate_limit_backoff = Duration::from_secs(
        env::var("MERGE_RPC_RATE_LIMIT_BACKOFF_SECS")
            .ok()
            .and_then(|s| s.trim().parse().ok())
            .unwrap_or(RPC_RATE_LIMIT_BACKOFF_DEFAULT),
    );
    let delay_between_markets = Duration::from_secs(
        env::var("MERGE_RPC_DELAY_SECS")
            .ok()
            .and_then(|s| s.trim().parse().ok())
    ...
    loop {
        ...
        for (i, &condition_id) in condition_ids.iter().enumerate() {
            if i > 0 {
                sleep(delay_between_markets).await;
            }
            ...
            let col_yes = match client.collection_id(&req_col_yes).await {
                Ok(c) => c,
                Err(e) => {
                    let msg = e.to_string();
            ...
            let col_no = match client.collection_id(&req_col_no).await {
                Ok(c) => c,
                Err(e) => {
                    let msg = e.to_string();
                    if msg.contains("rate limit") || msg.contains("retry in") {
            ...
            let req_pos_no = PositionIdRequest::builder()
                ...
                .build();
            let pos_yes = match client.position_id(&req_pos_yes).await {
                Ok(p) => p,
                Err(e) => {
                    let msg = e.to_string();
                    if msg.contains("rate limit") || msg.contains("retry in") {
            ...
            let pos_no = match client.position_id(&req_pos_no).await {
                Ok(p) => p,
                Err(e) => {
                    let msg = e.to_string();
                    if msg.contains("rate limit") || msg.contains("retry in") {
    ...
    for (cid, amt) in &merged_items {
        info!("  - condition_id={:#x} 数量:{} ({} USDC)", cid, amt, amt / U256::from(1_000_000));
    }
