...
/// 解析滑点数组：逗号分隔，如 "0,0.01" 或 "-0.01,-0.01"。
/// 索引 0=first（发送顺序中单价较高者），1=second（单价较低者）。
/// 只写一个值时用于两项，如 "0.01" 即 [0.01, 0.01]。默认 "0,0.01"。
...
pub struct Config {
    ...
    /// 滑点 [first, second]：按发送顺序，first=单价较高者，second=单价较低者。如 "0,0.01" 或 "-0.01,-0.01"
    pub slippage: [f64; 2],
